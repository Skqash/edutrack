...
<div class="container-fluid my-4">
    <div class="row mb-4">
        <div class="col-md-8">
            ...
            <p class="text-muted">Subject: {{ $class->subject->name ?? 'N/A' }} | Term: <strong>{{ ucfirst($term) }}</strong></p>
        </div>
                    ...
                    <table class="table table-hover mb-0">
                        <thead class="table-light">
                            ...
                            <tr>
                                ...
                                <!-- Final -->
                                <th class="text-center" style="background-color: #f0e8f8;"><small><strong>Grade</strong></small></th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="avatar bg-primary text-white rounded-circle d-flex align-items-center justify-content-center" style="width: 32px; height: 32px; font-size: 12px; font-weight: bold;">
                                                {{ substr($student->user->first_name, 0, 1) }}
                                            </div>
                                            <div>
                                                <small><strong>{{ $student->user->first_name }} {{ $student->user->last_name }}</strong></small><br>
                                    ...
                                    <!-- Skills Inputs -->
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][output_score]" class="form-control form-control-sm text-center"
                                            value="{{ old("grades.$index.output_score", $grade->output_score ?? '') }}"
                                            min="0" max="{{ $range->output_max ?? 100 }}"
                                            placeholder="0" step="0.5">
                                    </td>
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][class_participation_score]" class="form-control form-control-sm text-center"
                                    ...
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][activities_score]" class="form-control form-control-sm text-center"
                                            value="{{ old("grades.$index.activities_score", $grade->activities_score ?? '') }}"
                                            min="0" max="{{ $range->activities_max ?? 100 }}"
                                            placeholder="0" step="0.5">
                                    </td>
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][assignments_score]" class="form-control form-control-sm text-center"
                                    ...
                                    <!-- Attitude Inputs -->
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][behavior_score]" class="form-control form-control-sm text-center"
                                            value="{{ old("grades.$index.behavior_score", $grade->behavior_score ?? '') }}"
                                            min="0" max="{{ $range->behavior_max ?? 100 }}"
                                            placeholder="0" step="0.5">
                                    </td>
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][awareness_score]" class="form-control form-control-sm text-center"
                                            value="{{ old("grades.$index.awareness_score", $grade->awareness_score ?? '') }}"
                                            min="0" max="{{ $range->awareness_max ?? 100 }}"
                                            placeholder="0" step="0.5">
                                    </td>
                                    ...
                                    <!-- Attendance -->
                                    <td class="text-center">
                                        <input type="number" name="grades[{{ $index }}][attendance_score]" class="form-control form-control-sm text-center"
                                    ...
                                    <td class="text-center">
                                        <textarea name="grades[{{ $index }}][remarks]" class="form-control form-control-sm" placeholder="Remarks" rows="1" style="font-size: 11px;">{{ old("grades.$index.remarks", $grade->remarks ?? '') }}</textarea>
                                    </td>
                                    ...
                                    <td class="text-center" style="background-color: #f0e8f8;">
                                        @if($grade && $grade->final_grade)
                                            <div>
                                                <strong class="text-primary">{{ number_format($grade->final_grade, 2) }}</strong><br>
                                                <span class="badge bg-{{ \App\Models\Grade::getGradeColor($grade->final_grade) }}">{{ $grade->grade_letter }}</span>
                                            </div>
                                        @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="20" class="text-center py-4">
                                        <p class="text-muted mb-0"><i class="fas fa-inbox"></i> No students enrolled in this class</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
