        ...
        @if (empty($classGradeSummaries))
            ...
            <div class="empty-state">
                ...
                @else
                    ...
                    <a href="{{ route('teacher.grades') }}" class="btn btn-primary mt-3">
                        <i class="fas fa-edit"></i> Go to Grade Entry
                    </a>
                @endif
            </div>
        @else
            ...
            <div class="d-flex justify-content-between align-items-center mb-3 no-print">
                <div class="summary-info-badge">
                    ...
                    <span>Showing {{ count($classGradeSummaries) }} {{ count($classGradeSummaries) === 1 ? 'Class' : 'Classes' }}</span>
                </div>
                @if($selectedClassId)
            ...
            <!-- Class Summaries -->
            @foreach ($classGradeSummaries as $summary)
                <div class="class-summary-card">
                    <!-- Class Header -->
                    <div class="class-header">
                        <div class="class-title">{{ $summary['class']->class_name }}</div>
                        <div class="class-info">
                            <span><i class="fas fa-book"></i> {{ $summary['course']->program_name ?? 'N/A' }}</span>
                            <span><i class="fas fa-users"></i> {{ $summary['stats']['total_students'] }} Students</span>
                            <span><i class="fas fa-check-circle"></i> {{ $summary['stats']['graded_students'] }} Graded</span>
                        </div>
                    </div>
                                    ...
                                    @forelse($summary['students'] as $student)
                                        <tr>
                                            ...
                                            <td class="student-name-cell">
                                                ...
                                                <div style="font-size: 0.75rem; color: #64748b;">ID: {{ $student['student_id'] }}</div>
                                            </td>

                                            <!-- Midterm KSA Components -->
                                            <td class="grade-value {{ $student['midterm_k_class'] }}">
                                                {{ number_format($student['midterm_knowledge'], 2) }}
                                                <div class="component-breakdown">Exam + Quizzes</div>
                                            </td>
                                            <td class="grade-value {{ $student['midterm_s_class'] }}">
                                                {{ number_format($student['midterm_skills'], 2) }}
                                                <div class="component-breakdown">Output + Activities</div>
                                            </td>
                                            <td class="grade-value {{ $student['midterm_a_class'] }}">
                                                {{ number_format($student['midterm_attitude'], 2) }}
                                                <div class="component-breakdown">Behavior + Awareness</div>
                                            </td>

                                            <!-- Midterm Grade -->
                                            <td class="grade-value {{ $student['midterm_grade_class'] }}" style="background: #fef9e7; font-weight: 700;">
                                                {{ number_format($student['midterm_grade'], 2) }}
                                            </td>

                                            <!-- Midterm Decimal Grade -->
                                            <td style="text-align: center; font-weight: 600;">
                                                {{ number_format(\App\Helpers\GradeHelper::convertToDecimalScale($student['midterm_grade']), 2) }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                ...
                                <tbody>
                                    @forelse($summary['students'] as $student)
                                        <tr>
                                            ...
                                            <td class="grade-value {{ $student['final_k_class'] }}">
                                                {{ number_format($student['final_knowledge'], 2) }}
                                                <div class="component-breakdown">Exam + Quizzes</div>
                                            </td>
                                            <td class="grade-value {{ $student['final_s_class'] }}">
                                                {{ number_format($student['final_skills'], 2) }}
                                                <div class="component-breakdown">Output + Activities</div>
                                            </td>
                                            <td class="grade-value {{ $student['final_a_class'] }}">
                                                {{ number_format($student['final_attitude'], 2) }}
                                                <div class="component-breakdown">Behavior + Awareness</div>
                                            </td>

                                            <!-- Final Grade -->
                                            <td class="grade-value {{ $student['final_grade_class'] }}" style="background: #fff5e6; font-weight: 700;">
                                                {{ number_format($student['final_grade'], 2) }}
                                            </td>

                                            <!-- Final Decimal Grade -->
                                            <td style="text-align: center; font-weight: 600;">
                                                {{ number_format(\App\Helpers\GradeHelper::convertToDecimalScale($student['final_grade']), 2) }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                ...
                                <tbody>
                                    @forelse($summary['students'] as $student)
                                        <tr>
                                            ...
                                            <!-- Midterm Grade -->
                                            <td class="grade-value {{ $student['midterm_grade_class'] }}" style="text-align: center;">
                                                {{ number_format($student['midterm_grade'], 2) }}
                                            </td>

                                            <!-- Midterm Contribution -->
                                            <td style="text-align: center; font-weight: 600; color: #92400e;">
                                                {{ number_format($student['midterm_grade'] * 0.40, 2) }}
                                            </td>

                                            <!-- Final Grade -->
                                            <td class="grade-value {{ $student['final_grade_class'] }}" style="text-align: center;">
                                                {{ number_format($student['final_grade'], 2) }}
                                            </td>

                                            <!-- Final Contribution -->
                                            <td style="text-align: center; font-weight: 600; color: #9a3412;">
                                                {{ number_format($student['final_grade'] * 0.60, 2) }}
                                            </td>

                                            <!-- Overall Grade -->
                                            <td class="final-grade-cell {{ $student['overall_grade_class'] }}" style="font-size: 1.1rem;">
                                                {{ number_format($student['overall_grade'], 2) }}
                                            </td>

                                            <!-- Decimal Grade -->
                                            <td style="text-align: center; font-weight: 700; font-size: 1rem;">
                                                {{ $student['decimal_grade'] }}
                                            </td>
