...
@section('content')
    ...
    <div class="container-fluid py-4" style="max-width: 100%; min-width: 0; margin-top: 80px;">
        <div class="card shadow-sm" style="max-width: 100%; overflow: hidden;">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-0">📝 Grade Entry Form</h4>
                    <small class="text-muted">{{ $class->name }} - {{ ucfirst($term) }} Term</small>
                </div>
                <div>
            ...
            <form method="POST"
                action="{{ route('teacher.grades.store', ['classId' => $class->id]) }}?term={{ $term }}"
                id="gradeForm">
                @csrf

                <div class="card-body overflow-hidden" style="padding-top: 1.5rem;">
                    <div class="alert alert-info mb-3">
                        <strong>💡 Instructions:</strong> Enter all grades for the <strong>{{ ucfirst($term) }}</strong>
                        term. All averages will compute automatically.
                    </div>

                    <!-- KSA Color Legend -->
                    <div class="ksa-legend mb-4 p-3 bg-white border rounded" style="margin-left: 0; margin-right: 0;">
                        ...
                        <div class="ksa-legend-item">
                            ...
                            <span><strong>Skills (S):</strong> Output, Participation, Activities & Assignments</span>
                        </div>
                        <div class="ksa-legend-item">
                    ...
                    @if ($range)
                        <div class="alert alert-success">
                            <strong>📊 Current Assessment Ranges for {{ ucfirst($term) }} Term:</strong>
                            <div class="row mt-2">
                                <div class="col-md-6">
                                    <small><strong>Knowledge:</strong></small><br>
                                    <small>• Quizzes: {{ $range->quiz_1_max ?? 25 }} pts each</small><br>
                                    <small>• Midterm Exam: {{ $range->midterm_exam_max ?? 70 }} pts</small><br>
                                    <small>• Final Exam: {{ $range->final_exam_max ?? 80 }} pts</small>
                                </div>
                                <div class="col-md-6">
                                    <small><strong>Skills & Attitude:</strong></small><br>
                                    <small>• Output:
                                        {{ $term == 'midterm' ? $range->output_midterm ?? 30 : $range->output_final ?? 30 }}
                                        pts</small><br>
                                    <small>• Class Participation:
                                        {{ $term == 'midterm' ? $range->class_participation_midterm ?? 15 : $range->class_participation_final ?? 15 }}
                                        pts</small><br>
                                    <small>• Activities:
                                        {{ $term == 'midterm' ? $range->activities_midterm ?? 15 : $range->activities_final ?? 15 }}
                                        pts</small>
                                </div>
                            </div>
                            <small class="text-muted d-block mt-2">
                                ...
                                <a href="{{ route('teacher.assessment.configure', $class->id) }}"
                                    class="alert-link">Configure these ranges</a> to match your actual assessment items.
                            </small>
                        </div>
                    @else
                        <div class="alert alert-warning">
                            ...
                            <a href="{{ route('teacher.assessment.configure', $class->id) }}" class="alert-link">Configure
                                assessment ranges</a>
                        ...
                        <table class="table table-bordered table-sm text-center" style="min-width: 1400px;">
                            <thead class="table-light">
                                <tr class="table-primary text-white">
                                    <th class="text-start header-student">Student</th>
                            ...
                            <tbody>
                                <tr class="table-warning fw-semibold">
                                    ...
                                    @if ($term == 'midterm')
                                        <td title="Midterm Exam Max">{{ $range->midterm_exam_max ?? 100 }}</td>
                                    @else
                                        <td title="Final Exam Max">{{ $range->final_exam_max ?? 100 }}</td>
                                    @endif
                                    <td class="bg-info bg-opacity-25">-</td>
                                    <td title="Quiz 1 Max">{{ $range->quiz_1_max ?? 25 }}</td>
                                    <td title="Quiz 2 Max">{{ $range->quiz_2_max ?? 25 }}</td>
                                    <td title="Quiz 3 Max">{{ $range->quiz_3_max ?? 25 }}</td>
                                    <td title="Quiz 4 Max">{{ $range->quiz_4_max ?? 25 }}</td>
                                    <td title="Quiz 5 Max">{{ $range->quiz_5_max ?? 25 }}</td>
                                    <td class="bg-success bg-opacity-25 fw-bold">
                                        {{ ($range->quiz_1_max ?? 25) + ($range->quiz_2_max ?? 25) + ($range->quiz_3_max ?? 25) + ($range->quiz_4_max ?? 25) + ($range->quiz_5_max ?? 25) }}
                                    </td>
                                    <td class="bg-info bg-opacity-25">-</td>
                                    <td title="Output 1 Max">
                                        {{ $term == 'midterm' ? $range->output_midterm ?? 100 : $range->output_final ?? 100 }}
                                    </td>
                                    ...
                                    <td title="Class Part 1 Max">
                                        {{ $term == 'midterm' ? $range->class_participation_midterm ?? 100 : $range->class_participation_final ?? 100 }}
                                    </td>
                                    ...
                                    <td title="Activity 1 Max">
                                        {{ $term == 'midterm' ? $range->activities_midterm ?? 100 : $range->activities_final ?? 100 }}
                                    </td>
                                    ...
                                    <td title="Assignment 1 Max">
                                        {{ $term == 'midterm' ? $range->assignments_midterm ?? 100 : $range->assignments_final ?? 100 }}
                                    </td>
                                    ...
                                    <td title="Behavior 1 Max">
                                        {{ $term == 'midterm' ? $range->behavior_midterm ?? 100 : $range->behavior_final ?? 100 }}
                                    </td>
                                    ...
                                    <td class="bg-success bg-opacity-25 fw-bold">90</td>
                                    <td title="Attendance 1 Max">{{ $range->attendance_max ?? 100 }}</td>
                                    <td title="Attendance 2 Max">{{ $range->attendance_max ?? 100 }}</td>
                                    <td title="Attendance 3 Max">{{ $range->attendance_max ?? 100 }}</td>
                                    <td title="Awareness 1 Max">
                                        {{ $term == 'midterm' ? $range->awareness_midterm ?? 100 : $range->awareness_final ?? 100 }}
                                    </td>
                                ...
                                @foreach ($students as $student)
                                    ...
                                    <tr data-student-id="{{ $student->id }}">
                                        <td class="student-col text-start border-end bg-white fw-semibold">
                                            <div><strong>{{ $student->user->name ?? $student->name }}</strong></div>
                                        ...
                                        @if ($term == 'midterm')
                                            <td><input type="text" inputmode="numeric" pattern="[0-9]*"
                                                    data-min="0" data-max="{{ $range->midterm_exam_max ?? 100 }}"
                                                    class="form-control form-control-sm text-center grade-input exam-md"
                                                    name="grades[{{ $student->id }}][exam_md]"
                                                    value="{{ intval($entry->exam_md ?? 0) }}"></td>
                                        @else
                                            <td><input type="text" inputmode="numeric" pattern="[0-9]*"
                                                    data-min="0" data-max="{{ $range->final_exam_max ?? 100 }}"
                                                    class="form-control form-control-sm text-center grade-input exam-fn"
                                                    name="grades[{{ $student->id }}][exam_fn]"
                                                    value="{{ intval($entry->exam_fn ?? 0) }}"></td>
                                        @endif
                                        <td class="computed-cell exam-ave table-secondary fw-semibold">
                                            {{ $entry && $entry->exam_average !== null ? number_format($entry->exam_average, 2) : '-' }}
                                        </td>

                                        <!-- QUIZZES -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->quiz_1_max ?? 25 }}"
                                                class="form-control form-control-sm text-center grade-input quiz-1"
                                                name="grades[{{ $student->id }}][quiz_1]"
                                                value="{{ intval($entry->quiz_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->quiz_2_max ?? 25 }}"
                                                class="form-control form-control-sm text-center grade-input quiz-2"
                                                name="grades[{{ $student->id }}][quiz_2]"
                                                value="{{ intval($entry->quiz_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->quiz_3_max ?? 25 }}"
                                                class="form-control form-control-sm text-center grade-input quiz-3"
                                                name="grades[{{ $student->id }}][quiz_3]"
                                                value="{{ intval($entry->quiz_3 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->quiz_4_max ?? 25 }}"
                                                class="form-control form-control-sm text-center grade-input quiz-4"
                                                name="grades[{{ $student->id }}][quiz_4]"
                                                value="{{ intval($entry->quiz_4 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->quiz_5_max ?? 25 }}"
                                                class="form-control form-control-sm text-center grade-input quiz-5"
                                                name="grades[{{ $student->id }}][quiz_5]"
                                                value="{{ intval($entry->quiz_5 ?? 0) }}"></td>
                                        <td class="computed-cell quiz-total table-success fw-semibold">
                                            {{ $entry && ($entry->quiz_1 || $entry->quiz_2 || $entry->quiz_3 || $entry->quiz_4 || $entry->quiz_5) ? '100' : '-' }}
                                        </td>

                                        <!-- KNOWLEDGE AVERAGE -->
                                        <td class="computed-cell knowledge-ave table-secondary fw-semibold">
                                            {{ $entry && $entry->knowledge_average !== null ? number_format($entry->knowledge_average, 2) : '-' }}
                                        </td>

                                        <!-- OUTPUT -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->output_midterm ?? 100 : $range->output_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input output-1"
                                                name="grades[{{ $student->id }}][output_1]"
                                                value="{{ intval($entry->output_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->output_midterm ?? 100 : $range->output_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input output-2"
                                                name="grades[{{ $student->id }}][output_2]"
                                                value="{{ intval($entry->output_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->output_midterm ?? 100 : $range->output_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input output-3"
                                                name="grades[{{ $student->id }}][output_3]"
                                                value="{{ intval($entry->output_3 ?? 0) }}"></td>
                                        <td class="computed-cell output-total table-success fw-semibold">
                                            {{ $entry && ($entry->output_1 || $entry->output_2 || $entry->output_3) ? '100' : '-' }}
                                        </td>

                                        <!-- CLASS PART -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->class_participation_midterm ?? 100 : $range->class_participation_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input classpart-1"
                                                name="grades[{{ $student->id }}][classpart_1]"
                                                value="{{ intval($entry->classpart_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->class_participation_midterm ?? 100 : $range->class_participation_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input classpart-2"
                                                name="grades[{{ $student->id }}][classpart_2]"
                                                value="{{ intval($entry->classpart_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->class_participation_midterm ?? 100 : $range->class_participation_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input classpart-3"
                                                name="grades[{{ $student->id }}][classpart_3]"
                                                value="{{ intval($entry->classpart_3 ?? 0) }}"></td>
                                        <td class="computed-cell classpart-total table-success fw-semibold">
                                            {{ $entry && ($entry->classpart_1 || $entry->classpart_2 || $entry->classpart_3) ? '90' : '-' }}
                                        </td>

                                        <!-- ACTIVITIES -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->activities_midterm ?? 100 : $range->activities_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input activity-1"
                                                name="grades[{{ $student->id }}][activity_1]"
                                                value="{{ intval($entry->activity_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->activities_midterm ?? 100 : $range->activities_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input activity-2"
                                                name="grades[{{ $student->id }}][activity_2]"
                                                value="{{ intval($entry->activity_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->activities_midterm ?? 100 : $range->activities_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input activity-3"
                                                name="grades[{{ $student->id }}][activity_3]"
                                                value="{{ intval($entry->activity_3 ?? 0) }}"></td>
                                        <td class="computed-cell activity-total table-success fw-semibold">
                                            {{ $entry && ($entry->activity_1 || $entry->activity_2 || $entry->activity_3) ? '100' : '-' }}
                                        </td>

                                        <!-- ASSIGNMENTS -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->assignments_midterm ?? 100 : $range->assignments_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input assignment-1"
                                                name="grades[{{ $student->id }}][assignment_1]"
                                                value="{{ intval($entry->assignment_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->assignments_midterm ?? 100 : $range->assignments_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input assignment-2"
                                                name="grades[{{ $student->id }}][assignment_2]"
                                                value="{{ intval($entry->assignment_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->assignments_midterm ?? 100 : $range->assignments_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input assignment-3"
                                                name="grades[{{ $student->id }}][assignment_3]"
                                                value="{{ intval($entry->assignment_3 ?? 0) }}"></td>
                                        <td class="computed-cell assignment-total table-success fw-semibold">
                                            {{ $entry && ($entry->assignment_1 || $entry->assignment_2 || $entry->assignment_3) ? '50' : '-' }}
                                        </td>

                                        <!-- SKILLS AVERAGE -->
                                        <td class="computed-cell skills-ave table-secondary fw-semibold">
                                            {{ $entry && ($entry->output_1 || $entry->output_2 || $entry->output_3 || $entry->classpart_1 || $entry->classpart_2 || $entry->classpart_3 || $entry->activity_1 || $entry->activity_2 || $entry->activity_3 || $entry->assignment_1 || $entry->assignment_2 || $entry->assignment_3) ? number_format($entry->skills_average ?? 0, 2) : '-' }}
                                        </td>

                                        <!-- BEHAVIOR -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->behavior_midterm ?? 100 : $range->behavior_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input behavior-1"
                                                name="grades[{{ $student->id }}][behavior_1]"
                                                value="{{ intval($entry->behavior_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->behavior_midterm ?? 100 : $range->behavior_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input behavior-2"
                                                name="grades[{{ $student->id }}][behavior_2]"
                                                value="{{ intval($entry->behavior_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->behavior_midterm ?? 100 : $range->behavior_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input behavior-3"
                                                name="grades[{{ $student->id }}][behavior_3]"
                                                value="{{ intval($entry->behavior_3 ?? 0) }}"></td>
                                        <td class="computed-cell behavior-total table-success fw-semibold">
                                            {{ $entry && ($entry->behavior_1 || $entry->behavior_2 || $entry->behavior_3) ? '90' : '-' }}
                                        </td>

                                        <!-- ATTENDANCE (30% of Attitude engagement) -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->attendance_max ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input attendance-1"
                                                name="grades[{{ $student->id }}][attendance_1]"
                                                value="{{ intval($entry->attendance_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->attendance_max ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input attendance-2"
                                                name="grades[{{ $student->id }}][attendance_2]"
                                                value="{{ intval($entry->attendance_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $range->attendance_max ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input attendance-3"
                                                name="grades[{{ $student->id }}][attendance_3]"
                                                value="{{ intval($entry->attendance_3 ?? 0) }}"></td>

                                        <!-- AWARENESS -->
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->awareness_midterm ?? 100 : $range->awareness_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input awareness-1"
                                                name="grades[{{ $student->id }}][awareness_1]"
                                                value="{{ intval($entry->awareness_1 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->awareness_midterm ?? 100 : $range->awareness_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input awareness-2"
                                                name="grades[{{ $student->id }}][awareness_2]"
                                                value="{{ intval($entry->awareness_2 ?? 0) }}"></td>
                                        <td><input type="text" inputmode="numeric" pattern="[0-9]*" data-min="0"
                                                data-max="{{ $term == 'midterm' ? $range->awareness_midterm ?? 100 : $range->awareness_final ?? 100 }}"
                                                class="form-control form-control-sm text-center grade-input awareness-3"
                                                name="grades[{{ $student->id }}][awareness_3]"
                                                value="{{ intval($entry->awareness_3 ?? 0) }}"></td>
                                        <td class="computed-cell awareness-total table-success fw-semibold">
                                            {{ $entry && ($entry->awareness_1 || $entry->awareness_2 || $entry->awareness_3) ? '85' : '-' }}
                                        </td>

                                        <!-- ATTITUDE AVERAGE -->
                                        <td class="computed-cell attitude-ave table-secondary fw-semibold">
                                            {{ $entry && ($entry->behavior_1 || $entry->behavior_2 || $entry->behavior_3 || $entry->attendance_1 || $entry->attendance_2 || $entry->attendance_3 || $entry->awareness_1 || $entry->awareness_2 || $entry->awareness_3) ? number_format($entry->attitude_average ?? 0, 2) : '-' }}
                                        </td>
                    ...
                    <div
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary px-4">
                                <i class="fas fa-save me-1"></i> Save {{ ucfirst($term) }} Grades
                            </button>
                            <button type="button" class="btn btn-success px-4" data-bs-toggle="modal"
                                data-bs-target="#uploadModal">
                                <i class="fas fa-cloud-upload-alt me-1"></i> Upload
                            </button>
                            <a href="{{ route('teacher.grades.results') }}" class="btn btn-info px-4">
                ...
                <div class="card border-success shadow-lg">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info">
                            ...
                            <ul class="mb-0 mt-2">
                                <li>Transfer all saved {{ ucfirst($term) }} grades to the permanent
                    ...
                    <div class="modal-footer">
                        ...
                        <form method="POST" action="{{ route('teacher.grades.upload', ['classId' => $class->id]) }}"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculateRowAverages(row) {
                ...
                if (cells.length >= 2) {
                    ...
                    if (secondLastCell) {
                        secondLastCell.textContent = hasInput ? finalGrade.toFixed(2) : (secondLastCell.textContent === '-' ? '-' : secondLastCell.textContent);
                    }
                    if (lastCell && lastCell.classList.contains('decimal-grade-cell')) {
                        lastCell.textContent = hasInput ? decimalGrade.toFixed(2) : (lastCell.textContent === '-' ? '-' : lastCell.textContent);
                    }
                }
            }
        });
