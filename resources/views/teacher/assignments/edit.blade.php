...
@section('content')
    ...
    <div class="row mb-4">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h1 class="h3 fw-bold mb-0">Edit Assignment</h1>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('teacher.assignments.update', [$class->id, $assignment->id]) }}" method="POST">
                @csrf

                <div class="mb-3">
                    <label for="title" class="form-label">Assignment Title <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                        name="title" placeholder="Enter assignment title" value="{{ old('title', $assignment->title) }}"
                        required>
                    @error('title')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                        rows="4" placeholder="Enter assignment description" required>{{ old('description', $assignment->description) }}</textarea>
                    @error('description')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('instructions') is-invalid @enderror" id="instructions" name="instructions"
                        rows="3" placeholder="Enter assignment instructions (optional)">{{ old('instructions', $assignment->instructions) }}</textarea>
                    @error('instructions')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('max_score') is-invalid @enderror"
                                id="max_score" name="max_score" placeholder="100"
                                value="{{ old('max_score', $assignment->max_score) }}" min="1" max="1000"
                                required>
                            @error('max_score')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('term') is-invalid @enderror" id="term" name="term" required>
                        ...
                        <option value="midterm" {{ old('term', $assignment->term) === 'midterm' ? 'selected' : '' }}>
                            Midterm</option>
                        <option value="final" {{ old('term', $assignment->term) === 'final' ? 'selected' : '' }}>Final
                        </option>
                    </select>
                    @error('term')
                        <small class="text-danger">{{ $message }}</small>
