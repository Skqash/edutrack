        ...
        <div class="two-col mt-3">
            <div>
                ...
                <select name="academic_year" class="form-select" required>
                    ...
                    @endphp
                    @foreach($years as $yr)
                        <option value="{{ $yr }}" {{ old('academic_year', $years[1]) === $yr ? 'selected' : '' }}>{{ $yr }}</option>
                    @endforeach
                </select>
            </div>
            <div>
    ...
    <div class="form-card-body">
        <div class="mb-3">
            ...
            <select name="subject_id" class="form-select" id="subjectSelect" onchange="toggleNewSubject(this.value)">
                ...
                @endforeach
                <option value="new-subject" {{ old('subject_id') === 'new-subject' ? 'selected' : '' }}>
        ...
        {{-- New subject fields --}}
        <div id="newSubjectPanel" class="toggle-panel {{ old('subject_id') === 'new-subject' ? 'active' : '' }}">
                ...
                <div class="two-col mt-2">
                    <div>
                        ...
                        <select name="credit_hours" class="form-select">
                            @foreach([1,2,3,4,5,6] as $ch)
                                <option value="{{ $ch }}" {{ old('credit_hours', 3) == $ch ? 'selected' : '' }}>{{ $ch }} Credit Hour{{ $ch > 1 ? 's' : '' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="category" class="form-select">
                            @foreach(['Core','Major','Minor','Elective','General','Other'] as $cat)
    ...
    <div class="form-card-body">
        <div class="mb-3">
            ...
            <select name="course_id" class="form-select" id="courseSelect" onchange="toggleNewCourse(this.value)">
                ...
                @endforeach
                <option value="new-course" {{ old('course_id') === 'new-course' ? 'selected' : '' }}>
                    ➕ Create a new course
                </option>
            </select>
        ...
        {{-- New course fields --}}
        <div id="newCoursePanel" class="toggle-panel {{ old('course_id') === 'new-course' ? 'active' : '' }}">
                    ...
                    <label class="student-item" for="student_{{ $student->id }}">
                        ...
                        <div>
                            ...
                            <div style="font-size:.72rem;color:var(--muted)">
                                {{ $student->student_id }}
                                @if($student->course) · {{ $student->course->program_code }} @endif
    ...
    <div class="form-card-body">
        <div class="form-check mb-2">
            <input class="form-check-input" type="checkbox" name="auto_assign" value="1"
                   id="autoAssign" {{ old('auto_assign') ? 'checked' : '' }}>
...
<div class="d-flex gap-2 flex-wrap justify-content-end pb-4">
    <a href="{{ route('teacher.classes') }}" class="btn-secondary-custom">
...
document.addEventListener('DOMContentLoaded', () => {
    const oldIds = '{{ old('assigned_students', '') }}'.split(',').filter(Boolean);
    oldIds.forEach(id => {
        const cb = document.getElementById('student_' + id);
