                ...
                <form action="{{ route('admin.grades.update', $grade) }}" method="POST">
                    ...
                    <div class="form-row">
                        <div class="col-12 col-md-6 mb-3">
                            <label for="marks_obtained" class="form-label"><i class="fas fa-star"></i> Marks Obtained</label>
                            <input type="number" class="form-control @error('marks_obtained') is-invalid @enderror" id="marks_obtained" 
                                   name="marks_obtained" value="{{ $grade->marks_obtained }}" step="0.01" required>
                        ...
                        <div class="col-12 col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('total_marks') is-invalid @enderror" id="total_marks" 
                                   name="total_marks" value="{{ $grade->total_marks }}" step="0.01" required>
                            @error('total_marks')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select @error('grade') is-invalid @enderror" id="grade" name="grade" required>
                            ...
                            <option value="B" {{ $grade->grade == 'B' ? 'selected' : '' }}>B (80-89%)</option>
                            <option value="C" {{ $grade->grade == 'C' ? 'selected' : '' }}>C (70-79%)</option>
                            <option value="D" {{ $grade->grade == 'D' ? 'selected' : '' }}>D (60-69%)</option>
                            <option value="F" {{ $grade->grade == 'F' ? 'selected' : '' }}>F (Below 60%)</option>
                        </select>
                        @error('grade')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-row">
                        <div class="col-12 col-md-6 mb-3">
                            ...
                            <select class="form-select @error('semester') is-invalid @enderror" id="semester" name="semester" required>
                                ...
                                @for ($i = 1; $i <= 8; $i++)
                                    <option value="{{ $i }}" {{ $grade->semester == $i ? 'selected' : '' }}>Semester {{ $i }}</option>
                                @endfor
                            </select>
                            @error('semester')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="col-12 col-md-6 mb-3">
                            <label for="academic_year" class="form-label"><i class="fas fa-calendar-alt"></i> Academic Year</label>
                            <input type="text" class="form-control @error('academic_year') is-invalid @enderror" id="academic_year" 
                                   name="academic_year" value="{{ $grade->academic_year }}" required>
                            @error('academic_year')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
