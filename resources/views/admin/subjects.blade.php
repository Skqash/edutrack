                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <div style="flex: 1;">
                            <h6 class="text-muted text-uppercase fw-bold" style="font-size: 11px; letter-spacing: 0.5px;">
                                Categories</h6>
                            <h2 class="mb-0 fw-bold" style="color: #0d6efd;">{{ $subjects->pluck('category')->unique()->count() }}</h2>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <div style="flex: 1;">
                            <h6 class="text-muted text-uppercase fw-bold" style="font-size: 11px; letter-spacing: 0.5px;">
                                Programs</h6>
                            <h2 class="mb-0 fw-bold" style="color: #fd7e14;">{{ $subjects->pluck('program')->unique()->count() }}</h2>
                        </div>
        ...
        <div class="card-header bg-white border-0">
            <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    ...
                    @if($program !== 'General Education' && $programSubjects->first()->course)
                        <small class="text-muted ms-3">({{ $programSubjects->first()->course->program_code ?? 'N/A' }})</small>
                    @endif
                </div>
                <div class="d-flex align-items-center gap-3">
                    <div class="d-flex gap-2">
                        ...
                        <span class="badge bg-primary">{{ $programSubjects->where('type', 'Core')->count() }} Core</span>
                    </div>
                    @if($program !== 'General Education')
                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="syncProgramSubjects('{{ $program }}')" title="Sync subjects in this program">
            ...
            <div class="table-responsive">
                ...
                <tbody>
                    @foreach($programSubjects as $subject)
                        <tr>
                            <td><strong class="text-dark">{{ $subject->subject_code }}</strong></td>
                            <td>
                                <div>
                                    <strong class="text-dark">{{ $subject->subject_name }}</strong>
                                    <br><small class="text-muted">{{ $subject->description ?? 'Learn fundamental concepts and principles' }}</small>
                                </div>
                            </td>
                            ...
                            <td><span class="badge bg-{{ $subject->type == 'Core' ? 'primary' : ($subject->type == 'Elective' ? 'warning' : 'info') }}">{{ $subject->type ?? 'General' }}</span></td>
                            <td>
                                @if($subject->instructor)
                                    <span class="badge bg-light text-dark border">{{ $subject->instructor->name }}</span>
                                @else
