...
<div class="content-wrapper">
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.teacher-assignments.update', $teacherAssignment) }}" method="POST" id="assignmentForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                                        <option value="active" {{ old('status', $teacherAssignment->status) == 'active' ? 'selected' : '' }}>Active</option>
                                        <option value="inactive" {{ old('status', $teacherAssignment->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                                        <option value="completed" {{ old('status', $teacherAssignment->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                                    </select>
                                    @error('status')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="academic_year" id="academic_year" class="form-select @error('academic_year') is-invalid @enderror" required>
                                        @foreach($academicYears as $year)
                                            <option value="{{ $year }}" {{ old('academic_year', $teacherAssignment->academic_year) == $year ? 'selected' : '' }}>
                                                {{ $year }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="semester" id="semester" class="form-select @error('semester') is-invalid @enderror" required>
                                        @foreach($semesters as $semester)
                                            <option value="{{ $semester }}" {{ old('semester', $teacherAssignment->semester) == $semester ? 'selected' : '' }}>
                                                {{ $semester }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="department" id="department" class="form-select @error('department') is-invalid @enderror">
                                        ...
                                        @foreach($departments as $department)
                                            <option value="{{ $department }}" {{ old('department', $teacherAssignment->department) == $department ? 'selected' : '' }}>
                                                {{ $department }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="class_id" id="class_id" class="form-select @error('class_id') is-invalid @enderror">
                                        ...
                                        @foreach($classes as $class)
                                            <option value="{{ $class->id }}" 
                                                    ...
                                                    {{ old('class_id', $teacherAssignment->class_id) == $class->id ? 'selected' : '' }}>
                                    ...
                                    <select name="course_id" id="course_id" class="form-select @error('course_id') is-invalid @enderror">
                                        ...
                                        @foreach($courses as $course)
                                            <option value="{{ $course->id }}" 
                                                    ...
                                                    {{ old('course_id', $teacherAssignment->course_id) == $course->id ? 'selected' : '' }}>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="hidden" name="student_ids" id="student_ids" value='{{ old("student_ids", json_encode($teacherAssignment->students->pluck("id")->toArray())) }}'>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-12">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror" 
                                              rows="3" placeholder="Add any additional notes...">{{ old('notes', $teacherAssignment->notes) }}</textarea>
                                    @error('notes')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
