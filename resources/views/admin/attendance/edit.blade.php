                ...
                <form action="{{ route('admin.attendance.update', $attendance) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="date" class="form-control @error('date') is-invalid @enderror" id="date" 
                               name="date" value="{{ $attendance->date->format('Y-m-d') }}" required>
                        @error('date')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-3">
                        <label for="status" class="form-label"><i class="fas fa-check-square"></i> Attendance Status</label>
                    ...
                    <div class="form-group mb-3">
                        <label for="notes" class="form-label"><i class="fas fa-notes-medical"></i> Notes (Optional)</label>
                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" 
                                  name="notes" rows="3">{{ $attendance->notes }}</textarea>
                        @error('notes')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
