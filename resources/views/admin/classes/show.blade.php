...
@section('content')
    ...
    <script>
        ...
        document.getElementById('addStudentForm')?.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route('admin.classes.add-student-manually', $class->id) }}', {
                    method: 'POST',
                    headers: {
        ...
        document.getElementById('importExcelForm')?.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ route('admin.classes.import-excel', $class->id) }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content
                    },
        ...
        async function removeStudent(studentId) {
            try {
                const response = await fetch('{{ route('admin.classes.remove-student', $class->id) }}', {
                    method: 'DELETE',
                    headers: {
