        ...
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="studentAssignmentModalLabel">
                    ...
                    Assign Students to <span id="modalClassName">{{ $className ?? 'Class' }}</span>
                </h5>
                    ...
                    <div class="col-md-3">
                        ...
                        <select id="modal_course_filter" class="form-select form-select-sm">
                            ...
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}" data-department="{{ $course->department }}">
                                    {{ $course->program_code }} - {{ $course->program_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const loadStudents = () => {
            ...
            fetch('{{ route('admin.classes.get-students') }}', {
                    method: 'POST',
                    headers: {
        ...
        saveButton.addEventListener('click', () => {
            ...
            fetch('{{ route('admin.classes.assign-students') }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
