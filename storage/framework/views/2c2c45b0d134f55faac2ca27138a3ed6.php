    ...
    <div class="grade-system-container">
        ...
        <div class="system-header">
            <div>
                ...
                <p class="mb-0 opacity-75"><?php echo e($class->class_name); ?> - <?php echo e(ucfirst($term)); ?> Term</p>
            </div>
            <div class="d-flex gap-2">
                <a href="<?php echo e(route('teacher.grades.content', $class->id)); ?>" class="btn btn-light btn-sm">
                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h6 class="mb-2">
                            ...
                            Grade Entry for <?php echo e(ucfirst($term)); ?> Term
                        </h6>
                        <p class="mb-0 small">
                    ...
                    <div class="col-md-4 text-end">
                        <a href="<?php echo e(route('teacher.grades.content', $class->id)); ?>" class="btn btn-outline-primary btn-sm">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th colspan="2" class="knowledge-header">
                                <i class="fas fa-brain me-1"></i>Knowledge (<?php echo e(($ksaSettings->knowledge_weight ?? 0.4) * 100); ?>%)
                            </th>
                            
                            <!-- Skills Headers -->
                            <th colspan="2" class="skills-header">
                                <i class="fas fa-tools me-1"></i>Skills (<?php echo e(($ksaSettings->skills_weight ?? 0.5) * 100); ?>%)
                            </th>
                            
                            <!-- Attitude Headers -->
                            <th colspan="2" class="attitude-header">
                                <i class="fas fa-heart me-1"></i>Attitude (<?php echo e(($ksaSettings->attitude_weight ?? 0.1) * 100); ?>%)
                            </th>
                        ...
                        <tr>
                            ...
                            <?php if(isset($ksaSettings->components->knowledge)): ?>
                                <?php $__currentLoopData = $ksaSettings->components->knowledge; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $component): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <th class="knowledge-header"><?php echo e($component->name); ?> (<?php echo e($component->weight); ?>%)</th>
                            ...
                            <?php if(isset($ksaSettings->components->skills)): ?>
                                <?php $__currentLoopData = $ksaSettings->components->skills; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $component): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <th class="skills-header"><?php echo e($component->name); ?> (<?php echo e($component->weight); ?>%)</th>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php else: ?>
                            ...
                            <?php if(isset($ksaSettings->components->attitude)): ?>
                                <?php $__currentLoopData = $ksaSettings->components->attitude; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $component): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <th class="attitude-header"><?php echo e($component->name); ?> (<?php echo e($component->weight); ?>%)</th>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php else: ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="modalComponentWeight" class="form-control" min="0" max="100">
                </div>
                <div class="mb-3">
...
let componentConfigs = {
    knowledge: { 
        weight: <?php echo e(($ksaSettings->knowledge_weight ?? 0.4) * 100); ?>, 
        components: []
    },
    skills: { 
        weight: <?php echo e(($ksaSettings->skills_weight ?? 0.5) * 100); ?>, 
        components: []
    },
    attitude: { 
        weight: <?php echo e(($ksaSettings->attitude_weight ?? 0.1) * 100); ?>, 
        components: []
    }
};
...
function loadStudents() {
    const students = <?php echo json_encode($students->map(function($student) {
        return [
            'id' => $student->id, 'name' => trim(($student->first_name ?? '') . ' ' . ($student->last_name ?? '')) ?: 'Unknown'
        ];
    }), 512) ?>;
...
function calculateGrades(studentId) {
    ...
    ['knowledge', 'skills', 'attitude'].forEach(category => {
        ...
        finalScores[category] = categoryFinal; // Same for now, can be modified for different midterm/final components
    });
...
function getGradeStatus(score) {
    ...
    if (score >= 75) return { class: 'fair', label: 'Fair' };
    if (score >= 0) return { class: 'poor', label: 'Poor' };
    return { class: 'incomplete', label: 'Incomplete' };
}
...
function saveGrades() {
    ...
    // Send to server
    fetch(`/teacher/grades/advanced/<?php echo e($class->id); ?>/save-grades`, {
        method: 'POST',
        headers: {
...
function addComponent(category) {
    ...
    tbody.innerHTML = '';
    Object.keys(gradeData).forEach(studentId => {
        const student = { id: studentId, name: gradeData[studentId].name };
        const row = createStudentRow(student);
        tbody.appendChild(row);
    });
}
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.component-row input').forEach(input => {
...
function exportGrades() {
    ...
    Object.keys(gradeData).forEach(studentId => {
        ...
        ['knowledge', 'skills', 'attitude'].forEach(category => {
            componentConfigs[category].components.forEach(component => {
                const value = gradeData[studentId].components[`${category}_${component.index}`] || '';
                csv += `${value},`;
            });
        });
        csv += '\n';
...
<?php echo $__env->make('layouts.teacher', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\edutrack\resources\views/teacher/grades/advanced_grade_entry.blade.php ENDPATH**/ ?>