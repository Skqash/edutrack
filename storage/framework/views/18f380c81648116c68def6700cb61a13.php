<?php $__env->startSection('content'); ?>
    <div class="container-fluid px-2 px-md-3">
            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 small">
                        ...
                        <li class="breadcrumb-item active"><?php echo e($class->class_name ?? ($class->name ?? 'Class')); ?></li>
                    </ol>
                </nav>
            </div>
            <a href="<?php echo e(route('teacher.attendance')); ?>" class="btn btn-outline-secondary btn-sm">
        ...
        <?php else: ?>
            ...
            <form action="<?php echo e(route('teacher.attendance.record', $class->id)); ?>" method="POST" id="attendanceForm">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="date" id="dateInput" value="<?php echo e($today); ?>">
                <input type="hidden" name="term" id="termInput" value="<?php echo e($currentTerm); ?>">
                    ...
                    <div class="card-body py-3">
                        <div class="row align-items-center g-2">
                            <div class="col-12 col-md">
                                <div class="fw-bold text-primary">
                                    ...
                                    <?php echo e($class->class_name ?? ($class->name ?? 'Class')); ?>

                                </div>
                                <small class="text-muted">
                                    <?php if($class->program): ?>
                                        <?php echo e($class->program->program_name ?? ''); ?>

                                        <?php if($class->program->program_code): ?>
                                            (<?php echo e($class->program->program_code); ?>)
                                        <?php endif; ?>
                                    <?php else: ?>
                                        No program assigned
                                    <?php endif; ?>
                            ...
                            <div class="col-auto">
                                ...
                                <select id="termSelect" class="form-select form-select-sm fw-bold"
                                    ...
                                    <option value="Midterm" <?php echo e($currentTerm === 'Midterm' ? 'selected' : ''); ?>>
                                        📚 Midterm Term
                                    </option>
                                    <option value="Final" <?php echo e($currentTerm === 'Final' ? 'selected' : ''); ?>>
                                        🎓 Final Term
                                    </option>
                                </select>
                            </div>
                            <div class="col-auto">
                                ...
                                <input type="date" id="attendanceDate" class="form-control form-control-sm"
                                    value="<?php echo e($today); ?>" max="<?php echo e($today); ?>">
                            </div>
                        </div>
                        ...
                        <div class="mt-2">
                            <div class="alert alert-info py-2 px-3 mb-0" id="termIndicator">
                                ...
                                <span class="badge bg-primary ms-1" id="currentTermBadge"><?php echo e($currentTerm); ?> Term</span>
                        ...
                        <div class="mt-3 pt-3 border-top">
                            <div class="row g-2 small">
                                <div class="col-auto">
                                    <span class="text-muted">Total Meetings (<?php echo e($currentTerm); ?>):</span>
                                    <strong class="text-primary ms-1">
                                        <?php echo e($currentTerm === 'Midterm' ? $class->total_meetings_midterm : $class->total_meetings_final); ?>

                                    </strong>
                                </div>
                                <div class="col-auto">
                                    <span class="text-muted">Attendance Weight:</span>
                                    <strong class="text-success ms-1"><?php echo e($class->attendance_percentage); ?>%</strong>
                                </div>
                                <div class="col-auto">
                                    <a href="<?php echo e(route('teacher.attendance.settings', $class->id)); ?>"
                ...
                <?php if($studentsWithoutSignatures->count() > 0): ?>
                    <div class="alert alert-warning alert-dismissible fade show" role="alert" id="signatureAlert">
                        <div class="d-flex align-items-start">
                            <i class="fas fa-exclamation-triangle me-2 mt-1"></i>
                            <div class="flex-grow-1">
                                <strong>Missing E-Signatures (<span id="missingCount"><?php echo e($studentsWithoutSignatures->count()); ?></span> student<?php echo e($studentsWithoutSignatures->count() > 1 ? 's' : ''); ?>)</strong>
                                ...
                                <div class="small" id="missingStudentsList">
                                    <?php $__currentLoopData = $studentsWithoutSignatures->take(10); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <span class="badge bg-warning text-dark me-1 mb-1" data-student-id="<?php echo e($student->id); ?>">
                                            <i class="fas fa-user me-1"></i><?php echo e($student->first_name); ?> <?php echo e($student->last_name); ?>

                                        </span>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php if($studentsWithoutSignatures->count() > 10): ?>
                                        <span class="badge bg-secondary">+<?php echo e($studentsWithoutSignatures->count() - 10); ?> more</span>
                                    <?php endif; ?>
                                </div>
                ...
                <?php else: ?>
                    <div class="alert alert-success alert-dismissible fade show" role="alert" id="signatureAlert">
                        ...
                        <small class="d-block"><?php echo e($studentsWithSignatures); ?> of <?php echo e($students->count()); ?> students have captured their signatures.</small>
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                    </div>
                <?php endif; ?>
                ...
                <div class="student-list">
                    <?php $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $current = $attendances[$student->id] ?? null; ?>
                        <div class="student-card card border-0 shadow-sm mb-2">
                            <div class="card-body py-2 px-3">
                                <div class="d-flex align-items-center gap-3">
                                    
                                    <div class="student-num text-muted small fw-bold"><?php echo e($index + 1); ?></div>
                                    <div class="flex-grow-1 min-w-0">
                                        <div class="fw-semibold text-truncate"><?php echo e($student->name ?? 'N/A'); ?></div>
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-info signature-btn"
                                        ...
                                        data-student-name="<?php echo e($student->name ?? 'Student'); ?>"
                                        data-has-signature="<?php echo e($student->e_signature ? 'true' : 'false'); ?>"
                                        data-has-permanent-sig="<?php echo e(!empty($student->e_signature) ? '1' : '0'); ?>"
                                    ...
                                    <div class="status-btns d-flex gap-1">
                                        <label class="status-btn present-btn" title="Present">
                                            <input type="radio" name="attendance[<?php echo e($student->id); ?>][status]"
                                                value="Present" class="sr-only"
                                                <?php echo e($current && $current->status === 'Present' ? 'checked' : ''); ?>>
                                            <span><i class="fas fa-check"></i></span>
                                        </label>
                                        <label class="status-btn absent-btn" title="Absent">
                                            <input type="radio" name="attendance[<?php echo e($student->id); ?>][status]"
                                                value="Absent" class="sr-only"
                                                <?php echo e($current && $current->status === 'Absent' ? 'checked' : ''); ?>>
                                            <span><i class="fas fa-times"></i></span>
                                        </label>
                                        <label class="status-btn late-btn" title="Late">
                                            <input type="radio" name="attendance[<?php echo e($student->id); ?>][status]"
                                                value="Late" class="sr-only"
                                                <?php echo e($current && $current->status === 'Late' ? 'checked' : ''); ?>>
                                            <span><i class="fas fa-clock"></i></span>
                                        </label>
                                    </div>
                                    
                                    <input type="hidden" name="attendance[<?php echo e($student->id); ?>][e_signature]"
                ...
                <div class="d-flex justify-content-between align-items-center gap-2 mt-3 pb-3">
                    ...
                    <div class="d-flex gap-2">
                        <a href="<?php echo e(route('teacher.attendance')); ?>" class="btn btn-outline-secondary btn-sm">Cancel</a>
                        <a href="<?php echo e(route('teacher.attendance.sheet', ['classId' => $class->id])); ?>?date=<?php echo e($today); ?>&term=<?php echo e($currentTerm); ?>"
    ...
    <script>
        ...
        document.getElementById('termSelect').addEventListener('change', function() {
            ...
            if (confirm(`Switch to ${selectedTerm} term? Any unsaved attendance will be lost.`)) {
                ...
                this.value = '<?php echo e($currentTerm); ?>';
            }
        });

        // Initialize term indicator on page load
        document.addEventListener('DOMContentLoaded', function() {
            const currentTerm = '<?php echo e($currentTerm); ?>';
            const badge = document.getElementById('currentTermBadge');
            if (badge) {
...
<?php echo $__env->make('layouts.teacher', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\edutrack\resources\views/teacher/attendance/manage.blade.php ENDPATH**/ ?>